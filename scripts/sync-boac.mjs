...
function mapCategory(name, snippets) {
    ...
    if (text.includes('grocery') || text.includes('mart') || text.includes('supermarket')) return 'Retail / Shop';
    if (text.includes('hotel') || text.includes('resort') || text.includes('inn')) return 'Accommodation';
    if (text.includes('restaurant') || text.includes('food') || text.includes('kusina') || text.includes('casa')) return 'Restaurant';
...
async function syncResults() {
    const resultsPath = path.join(process.cwd(), 'boac_pilot_results.json');
    if (!fs.existsSync(resultsPath)) {
        console.error("No boac_pilot_results.json found.");
        return;
    }
    ...
    for (const biz of results) {
        ...
        const payload = {
            ...
            description: biz.description_snippet || `${biz.name} is a local establishment in Boac, Marinduque.`,
            categories: [category],
